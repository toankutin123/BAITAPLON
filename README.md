...
# Tạo file .env (chỉ cần khi cần AI features)
cp backend_py/.env.example backend_py/.env
# Edit backend_py/.env và thêm OPENROUTER_API_KEY nếu cần

# Start tất cả services (sẽ tự tạo database)
docker-compose up -d
```

### 2. Truy cập ứng dụng

Sau khi containers running:
...
- **Admin đầu tiên**: User đăng ký đầu tiên sẽ được automatic làm Admin
- Các user tiếp theo mặc định là Buyer
...
```bash
docker-compose down -v  # Xóa database volume
...
### AI Features
- AI price prediction cần `OPENROUTER_API_KEY`
- Lấy key free tại: https://openrouter.ai/
- Thêm vào `backend_py/.env`:
  ```
...
### Không cần API Key?
- App vẫn chạy được
- AI insights sẽ dùng fallback data
- Web scraping vẫn hoạt động

## Manual Setup (Không Docker)
